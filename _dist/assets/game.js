(function () {
  ...
  Game.prototype.build = function () {
    this.host.innerHTML = '<div class="tq_4e22ca7a"><div class="tq_92b0aefc" role="group" aria-label="Game mode"><button data-kind="daily">Daily</button><button data-kind="practice">Practice</button><button data-kind="mini">Mini 6</button></div><div class="tq_d06820d9"><span class="tq_66876af8">00:00</span><span class="tq_dd5225c1">Guess 1/6</span></div></div><div class="tq_c60784a4" role="grid" aria-label="Equation guesses"></div><p class="tq_44f96498" aria-live="polite">Enter a valid equation.</p><div class="tq_5d079a87" aria-label="Calculator keyboard"></div><div class="tq_13c883ca"></div>';
    this.board = this.host.querySelector(".tq_c60784a4"); this.message = this.host.querySelector(".tq_44f96498"); this.progress = this.host.querySelector(".tq_dd5225c1"); this.clock = this.host.querySelector(".tq_66876af8"); this.stats = this.host.querySelector(".tq_13c883ca");
    var self = this;
    this.host.querySelectorAll(".tq_92b0aefc button").forEach(function (b) { b.addEventListener("click", function () { self.mode = b.dataset.kind; self.length = self.mode === "mini" ? 6 : 8; self.start(); }); });
    var rows = [["7","8","9","/"],["4","5","6","*"],["1","2","3","-"],["0","=","DEL","+"],["ENTER"]];
    var kb = this.host.querySelector(".tq_5d079a87"); rows.forEach(function (row) { var div = document.createElement("div"); if (row.length === 1) div.style.gridTemplateColumns="1fr"; row.forEach(function (key) { var b = document.createElement("button"); b.type="button"; b.textContent=key === "ENTER" ? "SUBMIT" : key; if (key === "ENTER") { b.style.background="#a3e635"; b.style.letterSpacing=".08em"; } b.dataset.key=key; b.addEventListener("click", function(){ self.input(key); }); div.appendChild(b); }); kb.appendChild(div); });
  ...
  Game.prototype.start = function () {
    ...
    this.host.querySelectorAll(".tq_92b0aefc button").forEach(function(b){ b.classList.toggle("tq_0b7cbd5a", b.dataset.kind === this.mode); }, this);
  ...
  Game.prototype.notice=function(text,error){this.message.textContent=text;this.message.classList.toggle("tq_6cd92fb5",!!error);};
  ...
  Game.prototype.render=function(){
    ...
    this.host.querySelectorAll("[data-key]").forEach(function(b){b.classList.remove("tq_a2da04fc","present","absent");if(status[b.dataset.key])b.classList.add(status[b.dataset.key]);});
  };
